...
echo '#SBATCH --array=1-8			# Number of jobs in array' >> $1
...
echo '#SBATCH -oReport_%x_%A_%a.out                         # Combined output and error messages file' >> $1
echo '#SBATCH --mail-type=END,FAIL              # Mail preferences' >> $1
echo '#SBATCH --mail-user='$email'       # E-mail address for notifications' >> $1
echo '' >> $1
echo 'x=$(awk "NR==$SLURM_ARRAY_TASK_ID" x) ' >> $1
