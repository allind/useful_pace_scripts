#!/usr/bin/env python
#usage: script.py [fasta] [interpro] [deeploc] [dbcan]
...
def main(argv):
	...
	seqs = {seq.id: [[] for i in range(0,16)] for seq in SeqIO.parse(sys.argv[1], 'fasta')}
...
#			seqs[gene][18].append('-'.join(line.split('\t')[9:11]))
	...
	for line in deep:
		...
		ev = line.split(',')[2]
		name = loc + '-' + ev
		seqs[gene][15].append(name.strip('-'))
	

	dbcan = open(sys.argv[4])
	for line in dbcan:
		line = line.strip('\n')
		gene = line.split('\t')[0]
		annot = line.split('\t')[1]
		seqs[gene][14].append(annot)
	#print(seqs['KAH0796102.1'])
	#sys.exit()
	...
	print("Gene\tCDD-IPR\tGene3D\tHamap\tPANTHER\tPfam\tPIRSF\tPRINTS\tProSitePatterns\tProSiteProfiles\tSFLD\tSMART\tSUPERFAMILY\tTIGRFAM\tInterpro\tdbcan\tdeeploc")
	for seq in seqs:
		toprint = seq + '\t'
		for e in seqs[seq]:
